    ...
    <head>
		...
		$('#assignBundles').load('/config-ui/config/bundles/assignBundlesForm');
                ...
                $('[name="outgoing"]').change( function() {
                    ...
                    $.post('/config-ui/config/domain/updateBundleDirection',data, function(data) {
                        window.location.href = "/config-ui/config/domain?id=${domainName}#tab3";
                    });
                });
            ...
            $(function() {
                $('a[name=addMoreBundles]').click(function() {
                    $('#addMoreBundles').load('/config-ui/config/bundles/addMoreBundlesForm?domainName=${domainName}');
                });
            });
...
<thead>
                    ...
                    <c:forEach var="trustBundle" items="${trustBundles}" varStatus="rowCounter">
                        <c:choose>
                            ...
                            <td width=10><input name="bundlesToRemove" type="checkbox" value="${trustBundle.trustBundle.bundleName}"/></td>
                            <td>${trustBundle.trustBundle.bundleName}<br/>

                            </td>     
                            <td>                                  
                                 <a rel="leanModal" name="anchorList" href="#anchors_${trustBundle.trustBundle.bundleName}">View Anchors</a>
                                    <div id="anchors_${trustBundle.trustBundle.bundleName}" class="roundedCorners" style="display:none;background:white;padding:10px;width:350px;height:auto;">
                            ...
                            <td><input name="incoming" disabled="disabled" id="incoming_${trustBundle.trustBundle.bundleName}" type="checkbox" <c:if test="${trustBundle.incoming}">checked="true"</c:if>/></td>
                            <td><input name="outgoing" disabled="disabled" id="outgoing_${trustBundle.trustBundle.bundleName}" type="checkbox" <c:if test="${trustBundle.outgoing}">checked="true"</c:if>/></td>
                        </tr>
                    </c:forEach>
                    </table>
                    </div>                                
