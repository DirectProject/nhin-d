...
function updateDomainBundles() {
    ...
    if(bundleArrayString == "") {
        $('#noBundlesError').show().fadeOut(5000);
         ...
         $.post("/config-ui/config/domain/addBundle", { bundles: bundleArrayString}, function() {
        ...
        <table style="width:100%" class="fancyTable">
            ...
            <c:forEach var="trustBundle" items="${trustBundles}" varStatus="rowCounter">
                <c:choose>
                    ...
                    <td width=10><input name="bundlesToAssociate" type="checkbox" value="${encodedBundle}|${encodedBundle}"/></td>
                    
                    <td>${trustBundle.bundleName}</td>                                                     
                    <td><input name="new_incoming" id="new_incoming_${encodedBundle}" type="checkbox" checked /></td>
                    <td><input name="new_outgoing" id="new_outgoing_${encodedBundle}" type="checkbox" checked /></td>
                </tr>
            </c:forEach>
