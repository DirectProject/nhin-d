...
"%PHOENIX_JAVACMD%" %DEBUG% -classpath "%PHOENIX_HOME%/lib;%PHOENIX_HOME%/bin/phoenix-loader.jar" "-Djava.ext.dirs=%PHOENIX_HOME%\lib;%PHOENIX_HOME%\tools\lib" "-Dphoenix.home=%PHOENIX_HOME%" "-Djava.security.policy=jar:file:%PHOENIX_HOME%/bin/phoenix-loader.jar!/META-INF/java.policy" %PHOENIX_JVM_OPTS% %PHOENIX_SM% org.apache.avalon.phoenix.launcher.Main %1 %2 %3 %4 %5 %6 %7 %8 %9
